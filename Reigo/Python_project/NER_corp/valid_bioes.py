...
class HMM_BIOES:
    ...
    #训练模型(A,B,PI)
    #data (2,N)
    def train(self,filename,param_save_dir=None):
        ...
        self.save_Param(param_save_dir)
    ...
    def load_Param(self,filedir='param/main/'):
        self.A=np.genfromtxt(f'{filedir}A.csv',delimiter=',')
        self.B=np.genfromtxt(f'{filedir}B.csv',delimiter=',')
        self.PI=np.genfromtxt(f'{filedir}PI.csv',delimiter=',')
        print(f'训练参数(A,B,PI)已从 {filedir}A.csv 中读取成功')
        print()
...
#创建一个HMM模型用于命名实体识别
hmm_bio=HMM_BIOES()
hmm_bio.load_Param('param/BIOES/corpus_9G/')
...
# sql=f'select * from tbl_datas where rowid=={currow}'
# q=s3q.query(sql)[0]
...
rowmin=4
rowmax=1000
for currow in range(rowmin,rowmax+1):
    sql=f'select * from tbl_datas where rowid=={currow}'
    q=s3q.query(sql)[0][0]
    hash=q[0]
    text=q[2]
    ...
    for line in textList:
        ...
        while r<len(statuslist):
            l=r
            #如果当前状态不是O
            while r<len(statuslist) and statuslist[r]!=3:
                r+=1
            if r>l:
                nt_new_set.add(line[l:r])
            r+=1
    nt_new=";".join(list(nt_new_set))
    #print(nt_new)
    sql=f'UPDATE tbl_datas SET nt_new_bioes = \'{nt_new}\' WHERE hash = \'{hash}\';'
    print(s3q.exec(sql))
